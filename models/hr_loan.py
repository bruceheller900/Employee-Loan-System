...
class HrPayslip(models.Model):
    ...
    def onchange_employee_id(self, date_to=None, date_from=None):
        emp_id = self.employee_id
        lon_obj = self.env['hr.loan'].search([('employee_id', '=', emp_id.id), ('state', '=', 'approve')])
        hr_input_type = self.env['hr.payslip.input.type'].search([('name', '=', 'Loan')])
        for loan in lon_obj:
                ...
                if self.date_from <= loan_line.date <= self.date_to and not loan_line.paid:
                    for result in self.input_line_ids:
                        if result.code == 'LO':
                            ...
                            result['name'] = 'Loan'
                            result['input_type_id'] = hr_input_type.id
